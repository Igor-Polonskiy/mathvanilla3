(() => {
    ...
    const answersWrapper = document.querySelector('.task12_answers')
    const task12 = document.querySelector('.task12_wrapper')
    ...
    const answers = [{
        id: 1,
        data: '1',
        src: 'Images_3/assets8/task12_2.png'
    }]

    answers.forEach(item => {
        let answer = document.createElement('div')
        answer.classList.add('task12_answer')
    ...
    task12.addEventListener('mousedown', (e) => {
        ...
        if (e.target.classList.contains('task12_answer')) {
            ...
            draggingItem.classList.add('task12_answer_drop')
        ...
        if (e.target.classList.contains('task12_answer_drop')) {
            chek_answerTxt.innerHTML = ''
            checkTask.style.background = ''
            draggingItem = e.target
    ...
    document.addEventListener('mouseup', (e) => {
        if (draggingItem) {
            ...
            elemBelow = elemBelow.closest(".task12_dropeitem")

            if (elemBelow && elemBelow.classList.contains("task12_dropeitem")) {
    ...
    const dropeitems = document.querySelectorAll('.task12_dropeitem')
